...
class FourierSpaceVisualizer:
    ...
    def create_2d_arrays(led_converter, log=True, t_steps=23, f_steps=30):
        ff, tt = np.meshgrid(
            led_converter.frequency_arr[::f_steps],
    ...
    def plot_3d(
        ...
        ff, tt, zz = self.create_2d_arrays(
            led_converter=led_converter, log=log, t_steps=t_steps, f_steps=f_steps
        )

        if log:
            norm = None
        else:
            norm = colors.LogNorm(vmin=zz.min() * 1000, vmax=zz.max() * 1.0)
    ...
    def animate_3d(
        ...
        file_name = (
            f"{file_name}_t{t_steps}_f{f_steps}_{str_scale}_{cmap}_"
